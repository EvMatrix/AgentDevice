...
private:
    ...
    bool charging_pin_inited_ = false;
    int last_voltage_mv_ = -1;
    int trend_acc_ = 0;
    ...
    void InitializeADC() {
        ...
        if (adc_cali_create_scheme_curve_fitting(&cali_config, &adc1_cali_handle_) == ESP_OK) {
            do_calibration_ = true;
        }
#endif
    }

    void InitializeChargingDetect() {
        if (CHARGING_DETECT_PIN == GPIO_NUM_NC) {
            return;
        }
        gpio_config_t io_conf = {};
        io_conf.intr_type = GPIO_INTR_DISABLE;
        io_conf.mode = GPIO_MODE_INPUT;
        io_conf.pin_bit_mask = (1ULL << CHARGING_DETECT_PIN);
        io_conf.pull_down_en = GPIO_PULLDOWN_DISABLE;
        io_conf.pull_up_en = CHARGING_INPUT_PULLUP ? GPIO_PULLUP_ENABLE : GPIO_PULLUP_DISABLE;
        gpio_config(&io_conf);
        charging_pin_inited_ = true;
    }
...
public:
    ...
    virtual bool GetBatteryLevel(int& level, bool& charging, bool& discharging) override {
        ...
        if (do_calibration_) {
            ...
            voltage = voltage * VBAT_DIVIDER_RATIO;
            ...
            voltage = raw_value * VBAT_DIVIDER_RATIO;
            voltage = voltage < EMPTY_BATTERY_VOLTAGE ? EMPTY_BATTERY_VOLTAGE : voltage;
            voltage = voltage > FULL_BATTERY_VOLTAGE ? FULL_BATTERY_VOLTAGE : voltage;
            level = (voltage - EMPTY_BATTERY_VOLTAGE) * 100 / (FULL_BATTERY_VOLTAGE - EMPTY_BATTERY_VOLTAGE);
        }
        if (charging_pin_inited_) {
            charging = gpio_get_level(CHARGING_DETECT_PIN) == CHARGING_ACTIVE_LEVEL;
        } else {
            if (last_voltage_mv_ >= 0) {
                int delta = voltage - last_voltage_mv_;
                if (delta > 10) {
                    trend_acc_++;
                } else if (delta < -10) {
                    trend_acc_--;
                } else {
                    if (trend_acc_ > 0) trend_acc_--;
                    else if (trend_acc_ < 0) trend_acc_++;
                }
                if (trend_acc_ >= 3) {
                    charging_state_ = true;
                } else if (trend_acc_ <= -3) {
                    charging_state_ = false;
                }
            }
            last_voltage_mv_ = voltage;
